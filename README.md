...
| 特性 | 技术实现 |
|---|---|
| 模型协议 | 兼容 OpenAI 风格的 API (DeepSeek-Chat 等) |
| 上下文管理 | 基于 st.session_state 的动态长对话窗口 |
| 并发处理 | 设定了 3 轮最大迭代限制，防止由于逻辑死循环导致的 Token 异常消耗 |
| 系统兼容性 | 支持跨平台的 Shell 命令调度（适配 Windows/Linux） |
 4. 开发者总结
